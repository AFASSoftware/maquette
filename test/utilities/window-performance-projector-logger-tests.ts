...
describe('windowPerformanceProjectorLogger', () => {
  ...
  let clearCachedRequires = () => {
    let sourceFile = path.normalize(path.join(__dirname, '../../src/utilities/window-performance-projector-logger.ts'));
  ...
  let getLogger: () => ProjectorPerformanceLogger =
    () => require('../../src/utilities/window-performance-projector-logger.ts').windowPerformanceProjectorLogger;
  afterEach(() => {
